...
export const useSavedSearches = () => {
  ...
  const saveSavedSearch = useCallback(async (
    chain: ChunkChain,
    name: string,
    description?: string
  ): Promise<string | null> => {
    ...
    try {
      const savedSearchData = savedSearchService.chainToSaveData(chain, name, description);
      const savedSearchId = await savedSearchService.saveSavedSearch(user.uid, savedSearchData);
  ...
  const loadSavedSearch = useCallback(async (savedSearchId: string): Promise<ChunkChain | null> => {
    setLoading(true);
    setError(null);

    try {
      const savedSearch = await savedSearchService.getSavedSearchById(savedSearchId);
      if (!savedSearch) {
  ...
  const deleteSavedSearch = useCallback(async (savedSearchId: string): Promise<boolean> => {
    setLoading(true);
    setError(null);
