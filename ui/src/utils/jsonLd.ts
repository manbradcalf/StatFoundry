...
export function generatePersonJsonLd(player: PlayerInfo): JsonLdData {
...
export function generateWebsiteJsonLd(): JsonLdData {
  return {
    ...
    description: 'Advanced NFL player statistics search and analysis tool for fantasy football fans',
    potentialAction: {
      '@type': 'SearchAction',
      target: {
        '@type': 'EntryPoint',
...
export function generateBreadcrumbJsonLd(breadcrumbs: Array<{name: string, url: string}>): JsonLdData {
  return {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
...
export function injectJsonLd(data: JsonLdData | JsonLdData[]): string {
  ...
  return JSON.stringify(jsonData.length === 1 ? jsonData[0] : jsonData, null, 2);
}