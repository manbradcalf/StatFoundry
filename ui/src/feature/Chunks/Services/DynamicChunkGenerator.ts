...
export class DynamicChunkGenerator {
  ...
  private generateEntityMatchChunks(): Chunk[] {
    ...
    return this.schema.nodes.map((node) => ({
      ...
      Provides: [
        {
          Name: this.getVariableName(node.label),
          AliasType: node.label as any,
        },
      ],
      Slots: [],
    }));
  ...
  private getDefaultValue(property: string): any {
    ...
    if (property.includes("attempts") || property.includes("carries"))
      return 10;
    if (property.includes("receptions") || property.includes("targets"))
      return 5;
